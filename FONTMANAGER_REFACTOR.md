...
- `SetAlias` is simplified and renamed to `RegisterDocumentFontAlias`. The
  ...
  **Behaviour change:** because that face-copying loop is gone, an aliased
  ...
  the document — those lists filter on `face.documentId == document_id`, and
  the aliased font's faces remain registered globally (`documentId == -1`).
  Arguably more correct, since such fonts aren't actually embedded, but it's a
  user-visible difference from the old behaviour.
