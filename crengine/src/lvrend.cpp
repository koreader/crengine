    ...
    if ( is_floating || is_inline_box ) {
        ...
        if ( child->getNodeId() == el_img ) {
            ...
            if ( child_style->width.type == css_val_percent ) {
                ...
                newstyle->width.type = css_val_screen_px;
                newstyle->width.value = lengthToPx( enode, child_style->width, container_width );
            }
            if ( child_style->min_width.type == css_val_percent ) {
                ...
                newstyle->min_width.value = lengthToPx( enode, child_style->min_width, container_width );
            }
            if ( child_style->max_width.type == css_val_percent ) {
                ...
                newstyle->max_width.value = lengthToPx( enode, child_style->max_width, container_width );
            }
        }
        ...
        if ( ! BLOCK_RENDERING(flags, ALLOW_HORIZONTAL_BLOCK_OVERFLOW) ) {
            ...
            int child_margin_left   = lengthToPx( enode, child_style->margin[0], container_width );
            int child_margin_right  = lengthToPx( enode, child_style->margin[1], container_width );
