...
bool getStyledImageSize( ldomNode * enode, int & img_width, int & img_height, int container_width, int container_height, bool enforce_page_constraints ) {
    // We expect the <img>, <object>, <embed> HTML/EPUB elements or the <image> FB2/SVG element
    if ( !enode->isImage() )
        return false;
    // We may do specific things with <image>
    ...
    if ( nodeElementId == el_image && width < 0 && height < 0 ) {
