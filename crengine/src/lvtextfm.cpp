...
#define INLINEBOX_CHAR_INDEX  ((lUInt16)0xFFFD)
    ...
    int getCurrentLineUsableLeftOverflow() {
        int usable_left_overflow = m_usable_left_overflow;
        if (m_pbuffer->floatcount > 0) {
            // We might have left floats on this line, that
            // make m_usable_left_overflow no more relevant.
            // We'll allow the main text to overflow in these floats'
            // right margin (the float element content itself is also
            // allowed to overflow in it, so its margin is shared;
            // hopefully, both overflowing in it at the same position
            // will be rare).
            // Note that if the float that sets the text min x have some
            // large inward margin, an other further left float with
            // less inward margin might be the one that should limit
            // the usable overflow.
            int fl_left_max_x = 0;
            ...
            while (y <= end_y) {
                for (int i=0; i<m_pbuffer->floatcount; i++) {
                    ...
                    if (flt->to_position) // ignore not yet positionned floats
                        continue;
                    if (flt->is_right) // ignore right floats
                        continue;
                    if (flt->y <= y && flt->y + flt->height > y) { // this float is spanning this y
                        if (flt->x + flt->width > fl_left_max_x)
                            fl_left_max_x = flt->x + flt->width;
        ...
        return usable_left_overflow * USABLE_OVERFLOW_USABLE_RATIO;
    }
    int getCurrentLineUsableRightOverflow() {
        int usable_right_overflow = m_usable_right_overflow;
        if (m_pbuffer->floatcount > 0) {
            // We might have right floats on this line, that
            // make m_usable_right_overflow no more relevant.
            // We'll allow the main text to overflow in these floats'
            // left margin.
            int fl_right_min_x = m_pbuffer->width;
            ...
            while (y <= end_y) {
                for (int i=0; i<m_pbuffer->floatcount; i++) {
                    ...
                    if (flt->to_position) // ignore not yet positionned floats
                        continue;
                    if (!flt->is_right) // ignore left floats
                        continue;
                    if (flt->y <= y && flt->y + flt->height > y) { // this float is spanning this y
                        if (flt->x < fl_right_min_x)
                            fl_right_min_x = flt->x;
                        // The left margin of right floats is flt->inward_margin.
        ...
        return usable_right_overflow * USABLE_OVERFLOW_USABLE_RATIO;
    }
            ...
            if ( i>wstart && (   newSrc!=lastSrc
                ...
                if ( srcline->flags & LTEXT_SRC_IS_OBJECT ) {
                    ...
                    if ( firstWord && (align == LTEXT_ALIGN_LEFT || align == LTEXT_ALIGN_WIDTH) ) {
                        ...
                        if ( fit_glyphs ) {
                            ...
                            if ( m_hanging_punctuation && !(m_flags[wstart] & LCHAR_LOCKED_SPACING) ) {
                                // (We prevent it on the common opening quotation marks or dashes
                                // we flagged with LCHAR_LOCKED_SPACING - these may be candidate
                                // for hanging.)
                                bool check_font;
                                int percent = srcline->lang_cfg->getHangingPercent(false, check_font, m_text, wstart, end-wstart-1);
                                if ( percent ) {
                                    bool use_percent = true;
                                    if ( check_font && left_overflow > 0 ) {
                            ...
                            int usable_left_overflow = getCurrentLineUsableLeftOverflow();
                    ...
                    if ( lastWord && (align == LTEXT_ALIGN_RIGHT || align == LTEXT_ALIGN_WIDTH) ) {
                        ...
                        bool has_hyphen = m_flags[lastnonspace] & LCHAR_ALLOW_HYPH_WRAP_AFTER;
                        ...
                        if ( fit_glyphs ) {
                            ...
                            if ( m_hanging_punctuation ) {
                                if ( has_hyphen ) {
                                    ...
                                    if ( percent ) {
                                        bool use_percent = true;
                                        if ( check_font && right_overflow > 0 ) {
                                            // Some fonts might already have enough negative
                            ...
                            int usable_right_overflow = getCurrentLineUsableRightOverflow();
