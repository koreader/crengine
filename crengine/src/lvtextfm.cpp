...
#define MAX_WORD_SIZE 64
    ...
    void addLine( int start, int end, int x, src_text_fragment_t * para, bool first, bool last, bool preFormattedOnly, bool isLastPara, bool hasInlineBoxes )
    {
        ...
        // ones, where we'll handle it as some reserved space on the right.
        int rightIndent = 0;
        ...
        // XXX should we prevent hanging punctuation when there are some
        // floats on the left or right, and hanging might be drawn other
        // them ?
            ...
            if ( i>wstart && (   newSrc!=lastSrc
                ...
                if ( srcline->flags & LTEXT_SRC_IS_OBJECT ) {
                    ...
                    if ( firstWord && (align == LTEXT_ALIGN_LEFT || align == LTEXT_ALIGN_WIDTH) ) {
                        ...
                        // a some page might have multiple consecutive single lines that can
                        // benefit from hanging so the margin look clean too.
                        if ( m_hanging_punctuation && !(m_flags[wstart] & LCHAR_LOCKED_SPACING) ) {
                            // (We prevent if on the common opening quotation marks or dashes
                            // we flagged with LCHAR_LOCKED_SPACING, as they may be candidate
                            // for hanging.)
                            bool check_font;
                            ...
                            if ( percent ) {
                                ...
                                if ( check_font ) {
                                    ...
                                    if ( font->getLeftSideBearing(m_text[wstart]) < 0 )
                                        use_percent = false;
                                }
                                if ( use_percent ) {
                                    int first_char_width = m_widths[wstart] - (wstart>0 ? m_widths[wstart-1] : 0);
                                    int shift_x = first_char_width * percent / 100;
                                    if ( shift_x == 0 ) // Force at least 1px if division rounded it to 0
                                        shift_x = 1;
                                    frmline->x -= shift_x;
                                }
                            }
                            else { // If not some usual hanging char, remove any positive LSB
                                int lsb = font->getLeftSideBearing(m_text[wstart]);
                                if ( lsb > 0 )
                                    frmline->x -= lsb;
                                // Assume that when we want hanging punctuation, we can let italic
                                // overflowing on the left (like glyphs "J" or "f").
                            }
                        }
                        else {
                            // When we don't want anything hanging, correct for negative overflow.
                            ...
                            // and so leak on the left.
                            // But some regular font can have large one too.
                            // So, we do it in any case, whether negative or positive.
                            // Correcting that makes for nicer multilines higlighting boxes.
                            frmline->x -= font->getLeftSideBearing(m_text[wstart]);
                            // printf("%c lsb=%d\n", m_text[wstart], font->getLeftSideBearing(m_text[wstart]));
                            // Note: be sure lvrend.cpp getRenderedWidths() and lvtinydom.cpp
                            // ldomXPointer::getRect() use the same checks we do here.
                        }
                        // Should we account for any positive added shift, that we could remove
                        // if it makes us decrease the width of spaces too much ?
                    }
                    ...
                    if ( lastWord && (align == LTEXT_ALIGN_RIGHT || align == LTEXT_ALIGN_WIDTH) ) {
                        ...
                        // If we need to adjust last word last char correction,
                        // we need to put the delta in this word->width, which
                        // will make it into frmline->width.
                        ...
                        // Note: XXX it feels we should do that for the char before ANY image on the line (so the italic
                        ...
                        if ( m_flags[lastnonspace] & LCHAR_ALLOW_HYPH_WRAP_AFTER ) {
                            if ( m_hanging_punctuation ) {
                                // If the line ends with a hyphen, just hang the hyphen.
                                int percent = srcline->lang_cfg->getHyphenHangingPercent();
                                if ( percent ) {
                                    int shift_w = font->getHyphenWidth() * percent / 100;
                                    if ( shift_w == 0 ) // Force at least 1px if division rounded it to 0
                                        shift_w = 1;
                                    word->width -= shift_w;
                                }
                                // XXX Shouldn't we always take the hyphen width for the main
                                // paragraph font ? So that hyphens for parts of text in a
                                // bold font don't stand out more than others?
                            }
                        }
                        else if ( m_hanging_punctuation ) {
                            bool check_font;
                            ...
                            if ( percent ) {
                                ...
                                if ( check_font ) {
                                    ...
                                    if ( font->getRightSideBearing(m_text[lastnonspace]) < 0 )
                                        use_percent = false;
                                }
                                if ( use_percent ) {
                                    int last_char_width = m_widths[lastnonspace] - (lastnonspace>0 ? m_widths[lastnonspace-1] : 0);
                                    int shift_w = last_char_width * percent / 100;
                                    if ( shift_w == 0 ) // Force at least 1px if division rounded it to 0
                                        shift_w = 1;
                                    word->width -= shift_w;
                                }
                            ...
                            // and so leak on the right.
                            // But some regular font can have large one too.
                            // So, we do it in any case, whether negative or positive.
                            // Correcting that makes for nicer multilines higlighting boxes.
                            word->width -= font->getRightSideBearing(m_text[lastnonspace]);
                            // Note: be sure lvrend.cpp getRenderedWidths() and lvtinydom.cpp
                            // ldomXPointer::getRect() use the same checks we do here.
                        }
                    }
...
lUInt32 LFormattedText::Format(lUInt16 width, lUInt16 page_height, int para_direction, bool hanging_punctuation, BlockFloatFootprint * float_footprint)
{
    // clear existing formatted data, if any
