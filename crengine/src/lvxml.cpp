...
bool LVHTMLParser::CheckFormat()
{
    ...
    if (m_buf_len <= 30)
        return false;
    // At this stage, UTF-16/32 have been handled and returned. We haven't
    // supported any other ascii incompatible encodings, so it's safe to just
    // operate on bytes, as the following algorithm only cares about ascii data.
    // It is also what the standard do.
