    ...
    */
                     ...
                     )
    {
        ...
        if (_kerningMode == KERNING_MODE_HARFBUZZ) {
            ...
            bool use_fractional_positioning = fractionalGlyphPositioningMode == 16;
    ...
    */
    ...
    LVFontGlyphCacheItem * getGlyphByIndexSubpixel(lUInt32 index, int phase_64) {
        ...
        if (!item) {
            ...
            FT_Outline_Translate(&_slot->outline, phase_step * 4, 0);
                        ...
                        if (item) {
                            if ( transform_stretch ) {
                                ...
                                if ( (flags & LFNT_HINT_CJK_ALTERED_WIDTH) || (flags & LFNT_HINT_CJK_SCALED_WIDTH) || !use_fractional_positioning ) {
                                    ...
                                    int phase_step = (phase_64 + 2) / 4;
                                    if (phase_step >= 16) {
                                        phase_step = 0;
                                        draw_x += 1;
                                    }
                                    LVFontGlyphCacheItem *draw_item = getGlyphByIndexSubpixel(glyph_info[i].codepoint, phase_step * 4);
                                    if (!draw_item)
                                        draw_item = item;
                                    drawGlyphItem(buf,
