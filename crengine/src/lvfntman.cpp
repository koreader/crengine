...
void LVFontCache::_removeDocumentFonts(int documentId, LVPtrVector< LVFontCacheItem > &list)
{
    ...
    for (int i = 0; i < numDocumentFonts; ++i) {
        const LVFont *ptr = list[documentFonts[i]]->_fnt.get();
        if ( ptr == nullptr )
            continue;
        list[documentFonts[i]]->_fnt->clearFontRefs();
...
void LVFontCache::gc()
{
    ...
    for (int i=_instance_list.length()-1; i>=0; i--) {
        _instance_list[i]->_fnt->clearFontRefs();
        assert( _instance_list[i]->_fnt.getRefCount() >=1 );
