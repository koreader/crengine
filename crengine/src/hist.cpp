...
lString32 CRFileHistRecord::getLastTimeString( bool longFormat )
{
    ...
    char str[24];
    if ( !longFormat )
        snprintf(str, sizeof (str), "%02hhu.%02hhu.%04d", bt->tm_mday, 1+bt->tm_mon, 1900+bt->tm_year );
    else
        snprintf(str, sizeof (str), "%02hhu.%02hhu.%04d %02hhu:%02hhu", bt->tm_mday, 1+bt->tm_mon, 1900+bt->tm_year, bt->tm_hour, bt->tm_min);
